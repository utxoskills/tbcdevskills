...
| **MultiSig** | M-of-N multisig (1-6 sigs, 3-10 keys) | `createMultiSigWallet`, `p2pkhToMultiSig_sendTBC`, `buildMultiSigTransaction_sendTBC/transferFT`, sign + finish |
| **HTLC** | Hash time-lock (atomic swap) | `deployHTLC`, `withdraw`, `refund`, `fillSigDeploy`, `fillSigWithdraw`, `fillSigRefund` |
...
| **PiggyBank** | Time-locked savings | deposit, withdraw (after maturity), early withdraw (with penalty fee) |
...
- 赎回脚本: `OP_<M> <pubKey1> ... <pubKeyN> OP_<N> OP_CHECKMULTISIG`, 解锁需 `OP_0`（CHECKMULTISIG bug 补偿）+ M 个签名
...
```
0.3% fee (997/1000 factor). Underlying constant product: `x * y = k`.
Pool v2 adds `serviceRate` (default 25), `lpPlan` (1 or 2), optional lock time. Service provider: "bison".
LP token amount initialized equal to TBC amount. Precision: `BigInt(1000000)` (6 decimals for TBC).

Pool NFT outputs: `[index 0: Pool NFT (500 sat), index 1: LP Tape (0 sat), index 2: FT_A (500 sat), index 3: TBC (500 sat)]`
...
```
fillSigDeploy(deployHTLCTxRaw, sig, publicKey)     — 填充 deploy 交易签名
